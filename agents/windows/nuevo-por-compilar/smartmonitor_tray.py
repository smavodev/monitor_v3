...
def _load_tray_icon():
    """Carga el icono propio de SmartMonitor (instalado junto al .exe - ver
    [Files] en el .iss) - si no esta (build viejo, archivo movido/borrado a
    mano), cae al icono generico de Windows en vez de fallar."""
...
def show_pause_dialog():
    """Corre en su propio hilo (ver TrayIcon._on_command) - una ventana
    Tkinter por invocacion, se destruye sola al cerrar."""
    root = tk.Tk()
...
class TrayIcon:
    ...
    def __init__(self):
        ...
        try:
            ...
            pass  # ya registrada - no deberia pasar (un solo proceso vivo a la vez)
...
def main():
    if win32gui is None:
        return  # smoke-test de sintaxis fuera de Windows
    try:
        TrayIcon()
