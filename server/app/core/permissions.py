...
SECTIONS = {
    ...
    "areas": "Áreas",
    "parental_categories": "Categorías",
    "parental_blocked": "Bloqueo de sitios",
    "parental_schedule": "Horario",
    "parental_attempts": "Intentos bloqueados",
...
def get_section_level(db: Session, user, section: str) -> str:
    ...
    row = (
        ...
        .filter(
            RolePermission.role_id == user.role_id, RolePermission.section == section
        )
        .first()
    )
...
def has_permission(db: Session, user, section: str, min_level: str = "view") -> bool:
    return LEVEL_ORDER.get(get_section_level(db, user, section), 0) >= LEVEL_ORDER.get(
        min_level, 0
    )
